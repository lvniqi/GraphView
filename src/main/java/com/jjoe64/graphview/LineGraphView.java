    ...
    public void drawSeries(Canvas canvas, GraphViewDataInterface[] values, float graphwidth, float graphheight, float border, double minX, double minY, double diffX, double diffY, float horstart, GraphViewSeriesStyle style) {
        ...
        for (int i = 0; i < values.length; i++) {
            ...
            if (i > 0) {
                ...
                if(startY >border
                        && endY >border
                        && startY <graphheight+border
                        && endY <graphheight+border) {
                    // draw data point
                    if (drawDataPoints) {
                ...
                {
                    float temp_startx = startX;
                    float temp_starty = startY;
                    float temp_endx = endX;
                    float temp_endy = endY;
                    //if the starty is out of range
                    if(startY < border && endY >= border)
                    {
                        ...
                        temp_starty = border;
                        temp_endx = endX;
                        temp_startx = (border-startY)*dx/dy+startX;
                    }
                    //if the endy is out of range
                    else if(startY > border && endY <= border)
                    {
                        float dx = endX-startX;
                        float dy = endY-startY;
                        temp_endy = border;
                        temp_starty = startY;
                        temp_endx = (border-endY)*dx/dy+endX;
                        temp_startx = startX;
                    }
                    //if the starty is out of range
                    else if(startY >graphheight+border && endY <=graphheight+border)
                    {
                        float dx = endX-startX;
                        float dy = endY-startY;
                        temp_endy = endY;
                        temp_starty = graphheight+border;
                        temp_endx = endX;
                        temp_startx = (graphheight+border-startY)*dx/dy+startX;
                    }
                    //if the endy is out of range
                    else if(startY <=graphheight+border && endY >graphheight+border)
                    {
                        float dx = endX-startX;
                        float dy = endY-startY;
                        temp_endy = graphheight+border;
                        temp_starty = startY;
                        temp_endx = (graphheight+border-endY)*dx/dy+endX;
                        temp_startx = startX;
                    }
                    // draw data point
                    if (drawDataPoints) {
                    ...
                    if(
                            (startY >border || endY >border)
                            &&(startY <graphheight+border|| endY <graphheight+border)
                            ) {

                        canvas.drawLine(temp_startx, temp_starty, temp_endx, temp_endy, paint);
                        if (bgPath != null) {
                            if (i == 1) {
                                firstX = temp_startx;
                                bgPath.moveTo(temp_startx, temp_starty);
                            }
                            bgPath.lineTo(temp_endx, temp_endy);
                        }
                    }
                }
